...
class BinarySearchTree:
    ...
    def search(self, data):  # 有無找到: 有的話return True,  沒有的話 return False
        if data < self.data:
            is_find = self.left_tree.search(data) if self.left_tree else False
        elif data > self.data:
            is_find = self.right_tree.search(data) if self.right_tree else False
        else:
            # data == self.data
            is_find = True
...
if __name__ == "__main__":
    ...
    for target in targets_must_be_found:
        ...
        # 方法二
        assert bst.search(target) == True, print("Something goes wrong. by Assert")
